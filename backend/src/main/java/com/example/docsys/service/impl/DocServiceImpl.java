...
import com.example.docsys.search.SearchIndexService;
...
public class DocServiceImpl implements DocService {
    ...
    public DocServiceImpl(DocInfoMapper docInfoMapper, NlpClient nlpClient, SearchIndexService searchIndexService) {
    ...
    public List<DocInfo> search(SearchRequest request) {
        ...
        if (!StringUtils.hasText(request.getQ()) && !StringUtils.hasText(request.getCourseName()) && !StringUtils.hasText(request.getDocType())) {
            return docInfoMapper.selectList(new LambdaQueryWrapper<DocInfo>().orderByDesc(DocInfo::getUpdatedAt).last("limit " + offset + "," + pageSize));
        }
        // 优先走 ES 检索，命中后再回表读取完整数据
        List<Long> ids = searchIndexService.searchDocIds(request.getQ(), request.getCourseName(), request.getDocType(), offset, pageSize);
        if (!ids.isEmpty()) {
            return docInfoMapper.selectBatchIds(ids);
        }
